...
def load_data(path=None):
    """Load the wine quality dataset, downloading it if missing.

    Returns a DataFrame with the raw columns plus ``quality_binary``
    (1 when quality >= 7, else 0).
    """
    if path is None:
...
def get_features(df):
    ...
    Excludes both the raw ``quality`` label and the derived
    ``quality_binary`` target so neither leaks into model features.
    """
    return [c for c in df.columns if c not in ("quality", "quality_binary")]


def binary_split(df, feats, test_size=0.2, random_state=42):
    """Build the binary classification train/test split (quality >= 7)."""
    if "quality_binary" not in df.columns:
        df = df.copy()
        df["quality_binary"] = (df["quality"] >= 7).astype(int)
    X = df[feats].values
...
def build_models():
    """Return the binary classification model zoo as an OrderedDict."""
    return OrderedDict(
        [
...
def evaluate_models(models, X_tr_s, y_tr, X_te_s, y_te):
    """Fit every model and return {name: {acc, prec, rec, f1, auc}}."""
    results = {}
...
def main():
    ...
    # Train/Test split
    X_tr, X_te, y_tr, y_te, X_tr_s, X_te_s, _ = binary_split(df, feats)
    step(f"Split: {X_tr.shape[0]} train, {X_te.shape[0]} test")

    # Models
    models = build_models()
    results = evaluate_models(models, X_tr_s, y_tr, X_te_s, y_te)
